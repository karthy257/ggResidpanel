...
helper_label <- function(type = NA, model) {
  if (!is.na(type)) {
    ...
    } else if (type == "stand.cond") {
      return("Standardized Conditional Residuals")
    } else if (type == "stand.mar") {
      return("Standardized Marginal Residuals")
    } else if (type == "response.cond") {
      return("Raw Conditional Residuals")
    } else if (type == "response.mar") {
      return("Raw Marginal Residuals")
    } else if (type == "pearson.cond") {
      return("Pearson Conditional Residuals")
    } else if (type == "pearson.mar") {
      return("Pearson Marginal Residuals")
    }
  } else {
    if (class(model)[1] == "lm") {
      return("Residuals")
